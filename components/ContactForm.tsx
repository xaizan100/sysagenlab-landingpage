  ...
  return (
    <section className="py-24 px-4 max-w-6xl mx-auto">
      <div className="bg-[#111] border-gradient-shadow rounded-[3rem] p-8 md:p-16 flex flex-col md:flex-row gap-16 relative overflow-hidden">
        {/* Glow effect reverted to purple */}
        <div className="absolute -bottom-20 -right-20 w-80 h-80 bg-purple-500/10 blur-[100px] rounded-full"></div>
        
        <div className="flex-1">
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 tracking-tighter">Ready to scale your revenue?</h2>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 group cursor-pointer" onClick={() => window.location.href = 'mailto:hello@sysagenlab.ai'}>
              ...
              <div>
                <p className="text-white font-bold">hello@sysagenlab.ai</p>
                <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Official Inquiries</p>
              </div>
            </div>
            <div className="flex items-center gap-4 group">
               <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-pink-500 group-hover:bg-pink-500 group-hover:text-white transition-all shadow-inner">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
              </div>
              <div>
                <p className="text-white font-bold">Fast support via Slack</p>
                <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">For Active Clients Only</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 relative z-10">
          {status === 'success' ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 bg-purple-500/10 border border-purple-500/30 rounded-3xl animate-fade-in min-h-[400px]">
              ...
              <h3 className="text-3xl font-bold text-white mb-3 tracking-tighter">Inquiry Received!</h3>
              <p className="text-gray-400 font-medium">Our growth strategist will review your application. Expect a reply within 2 hours.</p>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest">Current Challenges</label>
                <textarea 
                  required 
                  name="message"
